...
{
  options.lsp = {
    enable = mkEnableOption "Enables lsp";
    nix = mkEnableOption "Enables nix";
    elixir = mkEnableOption "Enables elixir";
    clojure = mkEnableOption "Enables clojure";
    rust = mkEnableOption "Enables rust";
  };

  config = mkIf (cfg.enable) {
    nvim.startPlugins = with pkgs.neovimPlugins;
      (withPlugins cfg.enable [ nvim-lspconfig ]) ++
      (withPlugins cfg.clojure [ conjure vim-sexp vim-sexp-mappings ]) ++
      (withPlugins cfg.elixir [ vim-elixir ]);
